...
import {
    lookup, company, designation, areaExpert, certification, university, instituiton, newsubject
...
export async function verifySubjects(profileSection: string, currCatId: string, currGradeId: string, subjectsArray: ISubjectList) {
    try {
        ...
        for (let subject of subjectsArray) {
            ...
            subjectRecord = await newsubject.findOne({
                name: new RegExp(`^${subject.name}$`, 'i')
            })
            if (!subjectRecord) {
                subjectRecord = await newsubject.create({
                    name: capitalizeString(subject.name),
                    cr_dts: new Date().toISOString(),
                    up_dts: new Date().toISOString()
                })
            }
            subject['id'] = subjectRecord['_id'];
            subject['name'] = subjectRecord['name'];
    ...
    try {
        ...
        for (let detail of profDetails.data) {
            ...
            if (!designationRecord) {
                designationRecord = await designation.create({
...
export const addGradeWiseChaptersAndTopics = async (profileObj) => {
    try {
        const user_id = profileObj.user_id.toString();
        const currCatId = profileObj.learning.curr_cat_id;
        const currGradeId = profileObj.learning.curr_grade_id;
        const subjects = profileObj.learning.subjects;
        let subjectId = [];
        for (let subject of subjects) {
            subjectId.push(subject.id)
        }
        return { user_id, currCatId, currGradeId, subjectId }
    } catch (error: any) {
        throw Error('addGradeWiseChaptersAndTopics' + error.message);
    }
}