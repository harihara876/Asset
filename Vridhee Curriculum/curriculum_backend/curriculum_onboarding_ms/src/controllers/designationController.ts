...
import { designation } from "../dbmanager/dbconnection";
import { getReusable } from "../services/reusableService";

export async function getDesignationData(req: Request, res: Response, next: NextFunction){
    try{
        let user = await getReusable(designation);
        if(user){
