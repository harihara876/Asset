...
export const uploadFileIntoS3 = async (folderName, id, image, uniqueName) => {
    let buf: Buffer
    ...
    // const extension = image.originalname
    let uName = '';
    if (uniqueName) {
        uName = '_' + uniqueName;
    }
    const key = `${folderName}/${id}${uName}.${extension}`;
...
export const deleteFileS3 = async (imageUrl: string) => {
    try {
        const params = {
            Bucket: configData.s3Bucket.bucket,
            Key: imageUrl,
