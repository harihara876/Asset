...
function Hobbiespassion() {
        ...
        if (localStorage.getItem('profileId')) {
            ...
            OnboardingService.getProfileData(profileId)
                .then(res => {
                    ...
                    if (addLabelInHobbies && addLabelInPassions) {
                        nextFlag = true
                    } else {
                        nextFlag = false
                    }
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        if (hvalues.length && pvalues.length) {
            const body = {
                "hobbies_passion": {
                    "hobbies": {
                        "data": hvalues
            ...
            if (!pvalues.length && !hvalues.length) {
                toast.error("Please select atleast one hobby and one passion")
            }
            else if (!hvalues.length) {
                toast.error("Please select atleast one hobby")
            }
            else if (!pvalues.length) {
                toast.error("Please select atleast one passion")
            }
    ...
    const onHobbyKeyDown = (e: any) => {
        if (!(/^[a-z][a-z\s]*$/i.test(e.key))) {
            e.preventDefault();
        }
    };
    const onPassionKeyDown = (e: any) => {
        if (!(/^[a-z][a-z\s]*$/i.test(e.key))) {
            e.preventDefault();
        }
    };
    ...
    function reset(e: React.MouseEvent<EventTarget>) {
        e.preventDefault();
        setHobbyDetail({
            ...hobbydetail, hobbydetails: []
        })
        setPassionDetail({
            ...passiondetail, passiondetails: []
        })
        setHValues([]);
        setPValues([]);
    }
                            ...
                            <div className="inner-page-content">
                                ...
                                <div className="person-name">
                                    ...
                                    <div className="right-end text-right">
                                        <h4 className="primary-color m-0">STEP 03/08</h4>
