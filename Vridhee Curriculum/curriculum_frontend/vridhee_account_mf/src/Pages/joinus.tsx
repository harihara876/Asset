...
export default function Joinus() {
    ...
    useEffect(() => {
        if( localStorage.getItem('actorType') != 'Campus'){
            LoginService.checkIsUserLogin()
            .then(res => {
                if (res.data.code == 200) {
                    if (res.data.data.accessToken) {
                        ...
                        localStorage.setItem('Tokenuserid', userdetails.user_id)
                            LoginService.userVerify(userId)
                                .then(result => {
                                    if (result.data.status == 200 && result.data.msg == 'Onboarding_Completed') {
                                        window.location.replace(`${Dashboard_Route_URL}dashboard` + "?" + localStorage.getItem('actorType')  + "?" + localStorage.getItem('sid') + "?"  + res.data.data._id )
                                        ...
                                        // window.location.replace(`${Route_URL}` + "?" + localStorage.getItem('actorType') + "?" + localStorage.getItem('sid') + "?"  + res.data.data._id )
