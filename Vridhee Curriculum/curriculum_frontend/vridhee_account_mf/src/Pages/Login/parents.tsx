import Grid from "@mui/material/Grid";
import React from "react";
import configJson from "vridhee_common_component_mf/configJson";

export default function ParentsN() {
    const assetUrl = configJson.local.assetUrl
    return (
        <div>
            <div className="user-con">
                <Grid container>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Most trustworthy & engaging multilingual digital content for Free</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box bg-white">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Every children is different so why same process, mentor & content for all</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box bg-white">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Studding along in boring so enable a learning journey of your kid with peers</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Doubt clearance, guidance & mentoring from global pool of teachers who can understand your kid personally</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Activity driven learning for academic, moral & cultural growth of your kid</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box bg-white">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Be a learner or mentor and engage to increase family bonding</p>
                        </div>
                    </Grid>
                </Grid>
            </div>
        </div>
    );
}