import Grid from "@mui/material/Grid";
import React from "react";
import configJson from "vridhee_common_component_mf/configJson";

export default function LearnN() {
    const assetUrl = configJson.local.assetUrl
    return (
        <div>
            <div className="user-con">
                <Grid container>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Earn while you learn in your preferred language</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Do not learn alone, socialize yourself with right network of friends and mentors</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Become more disciplined and focused learner along with fun activities</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Be a active learner by activity driven learning for academic, moral & cultural growth</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box primary-bg text-white">
                            <img src="data:image/png;base64," alt="" />
                            <p className="m-0">Share knowledge to gain knowledge.</p>
                        </div>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <div className="user-con-box">
                            <img src={`${assetUrl}/login/Recognization student.svg`} alt="" />
                            <p className="m-0">Win competitions to earn certificates & prizes</p>
                        </div>
                    </Grid>
                </Grid>
            </div>
        </div>
    );
}